...
clas_1=np.vstack([vals1_n,vals1a_n])
clas_2=np.vstack([vals2_n,vals2a_n])
clas_3=np.vstack([vals3_n,vals3a_n])
clas_4=np.vstack([vals4_n,vals4a_n])
clas_5=np.vstack([vals5_n,vals5a_n])
clas_6=np.vstack([vals6_n,vals6a_n])
...
dat=np.vstack([a,b,c,d,e,f]) 
lab=pd.concat([a_lab,b_lab,c_lab,d_lab,e_lab,f_lab])
X=dat
...
model.fit(X_train, y_train, nb_epoch=4, batch_size=128, validation_split=0.3)
time.sleep(2)
